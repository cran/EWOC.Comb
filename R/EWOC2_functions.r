...
# 2. Generate JAGS (BUGS) file 
  Function.generate.bugs.file <- function() { 
    temp_directory <- tempdir()  # Get the temporary directory path
  bug_file_path <- file.path(temp_directory, "combination_cohort_2.bug")  # Construct the full path for the .bug file
...
file = bug_file_path , fill = TRUE, sep = "")

return(bug_file_path)  # Return the path to the created .bug file
...
# 3. function to calculate the dose for next cohort of patients
  ...
  Function.nextdose.2d <- function(dose.a,dose.b,resp, theta, alpha, Min.Dose.A, Max.Dose.A, Min.Dose.B, Max.Dose.B, a01,b01,a10,b10,a00,b00,a,b, MDI.A, MDI.B, delta1x, delta1y, burn,mm, delta1) {
   ...
   # data
    ...
    # call jags
       j=jags.model(Function.generate.bugs.file(),data=list('Z'=Z,'X'=X,'Y'=Y,'theta'=theta,'a00'=a00,'b00'=b00,'a01'=a01,'b01'=b01,'a10'=a10,'b10'=b10,'a'=a,'b'=b,'N'=n),n.chains=chains,n.adapt=burn, quiet=TRUE)
...
j=jags.model(Function.generate.bugs.file(),data=list('Z'=Z,'X'=X,'Y'=Y,'theta'=theta,'a00'=a00,'b00'=b00,'a01'=a01,'b01'=b01,'a10'=a10,'b10'=b10,'a'=a,'b'=b,'N'=NN),n.chains=chains,n.adapt=burn, quiet=TRUE)
...
# ewoc functions:
    ...
    # 1.1 print function for ewoc2
      ...
      # input parameter checking
      if(!inherits(x, "ewoc2")) stop("wrong class, must be ewoc2")	
...
# ewoc functions for simulation:
   ...
   # input parameter checking
     if(!inherits(x, "ewoc2simu")) stop("wrong class, must be ewoc2simu")
   ...
   # read parameters
    ...
    # YC code: avoid round.a and round.b to be 0
    if (round.a == 0) round.a = 2
    if (round.b == 0) round.b = 2
...
# This program plots the true and estimated MTD of EWOC using drug combinations along with the last trial doses.
    ...
    on.exit(par(oldpar)) # Ensure graphical parameters are reset on exit
